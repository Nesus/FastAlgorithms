...
void mpi_initialize(cell *C, int NCRIT) {
  C->nleaf = C->nchild = 0;
  C->parent = NULL;
...
void mpi_add_child(int octant, cell *C, cell *&CN, int NCRIT) {
  ++CN;
  mpi_initialize(CN, NCRIT);
  CN->r  = C->r/2;
...
void mpi_split_cell(double *x, double *y, double *z, cell *C, cell *&CN,int NCRIT) {
  for( int i=0; i<NCRIT; i++ ) {
    int l = C->leaf[i];
    int octant = (x[l] > C->xc) + ((y[l] > C->yc) << 1) + ((z[l] > C->zc) << 2);
    if( !(C->nchild & (1 << octant)) ) mpi_add_child(octant,C,CN, NCRIT);
    cell *CC = C->child[octant];
    CC->leaf[CC->nleaf++] = l;
    if( CC->nleaf >= NCRIT ) mpi_split_cell(x,y,z,CC,CN, NCRIT);
  }
}

void mpi_getMultipole(cell *C, double *x, double *y, double *z, double *m, int NCRIT) {
  double dx,dy,dz;
  if( C->nleaf >= NCRIT ) {
    for( int c=0; c<8; c++ )
      if( C->nchild & (1 << c) ) mpi_getMultipole(C->child[c],x,y,z,m,NCRIT);
  } else {
...
void mpi_upwardSweep(cell *C, cell *P) {
  double dx,dy,dz;
  dx = P->xc-C->xc;
  dy = P->yc-C->yc;
  dz = P->zc-C->zc;
...
void mpi_evaluate(cell *C, double *x, double *y, double *z, double *m, double &p, int i, int NCRIT, double THETA, double EPS2) {
  ...
  if( C->nleaf >= NCRIT ) {
    for( int c=0; c<8; c++ )
      if( C->nchild & (1 << c) ) {
        ...
        if( CC->r > THETA*r ) {
          mpi_evaluate(CC,x,y,z,m,p,i, NCRIT, THETA, EPS2);
        } else {
          X = x[i]-CC->xc;
          Y = y[i]-CC->yc;
          Z = z[i]-CC->zc;
...
void c_mpi_treecode(double THETA,int NCRIT,int N, double* potential,double* x,double* y,double* z,double* m, double EPS2){
  int ID, nprocs;
  ...
  if(ID == 0){
    ...
    //ELIMINATE WHEN SOMETHING IS PARALLEL
    // Set root cell
    cell C0[N];
    mpi_initialize(C0,NCRIT);
    ...
    for( int i=0; i<N; i++ ) {
      ...
      C->leaf[C->nleaf++] = i;
      if( C->nleaf >= NCRIT ) mpi_split_cell(x,y,z,C,CN,NCRIT);
    }

    // Multipole expansion
    mpi_getMultipole(C0,x,y,z,m,NCRIT);
    // Upward translation
    ...
    for( int i=0; i<N; i++ ) {
      ...
      mpi_evaluate(C,x,y,z,m,p,i,NCRIT,THETA,EPS2);
      potential[i] = p;
    }
